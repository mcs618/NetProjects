...
def errordetect(ssh_channel, ip_addr):
    ...
    del shverlist[:2]
    del shverlist[-1]
    # for 3750  Tested with: 12.2(44))SE6
    if 'uptime' in shverlist[2]:
        del shverlist[:2]
    ...
    if imagetemp.startswith('/'):
        ...
        C3x50imgRegex = re.compile(r'(\*\s+\d+\s+\d+\s+)(WS-C3[68]50-\d+[A-Z]*\s+)(\d+\.\d+\.\w*)')
        mo = C3x50imgRegex.search(shverlist[4])
        image = mo.group(3)
    else:
        image = imagetemp[:-1]
    ...
    if shverlist[2].startswith('Model'):
    ...
    elif shverlist[4].startswith('Model'):  # 3750
        modellist = shverlist[4].split(': ')
        model = modellist[1]
        serialnumlist = shverlist[5].split(': ')
        serialnum = serialnumlist[1]
        ...
        serialnum = serialnumlist[1]
    elif shverlist[2].startswith('cisco WS-C3850') or shverlist[2].startswith('cisco WS-C3650'):
        C3x50Regex = re.compile(r'(WS-C3[68]50-\d+[A-Z]*)')
        mo = C3x50Regex.search(shverlist[2])
        model = mo.group(1)
        serialnumlist = shverlist[3].split('ID ')
        serialnum = serialnumlist[1]
    else:
    ...
    outputfile = ("d:\\python\\Projects\\error_detect\\" + filename)
...
def main():
    ...
    csvfile = "d:\python\projects\error_detect\SwitchData.csv"
